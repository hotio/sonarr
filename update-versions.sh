...
response_json=$(curl -fsSL "https://services.sonarr.tv/v1/releases" | jq -e '.["v4-stable"]')
...
jq --sort-keys \
    ...
    '.version = $version | .sbranch = $sbranch | .amd64_url = $amd64_url | .arm64_url = $arm64_url' <<< "${json}" | tee meta.json
