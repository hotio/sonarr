...
version=$(jq -r .version <<< "${response_json}")
sbranch=$(jq -r .branch <<< "${response_json}")
amd64_url=$(jq -r '.linuxMusl.x64.archive.url' <<< "${response_json}")
arm64_url=$(jq -r .linuxMusl.arm64.archive.url <<< "${response_json}")
